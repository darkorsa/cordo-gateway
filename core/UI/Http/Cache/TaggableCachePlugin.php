...
class TaggableCachePlugin implements Plugin
{
    ...
    /**
     * @param TaggableCacheItemPoolInterface       $pool
     ...
     *     @var string $tags_request_header request header name with comma separated cache item tags
     * }
     */
    public function __construct(TaggableCacheItemPoolInterface $pool, $streamFactory, array $config = [])
    {
        ...
        if (null === ($this->config['tags_request_header'] ?? null)) {
            $this->config['tags_request_header'] = 'Request-Cache-Tags';
        }
    }
     ...
     * @return CachePlugin
     */
    public static function clientCache(TaggableCacheItemPoolInterface $pool, $streamFactory, array $config = [])
    {
     ...
     * @return CachePlugin
     */
    public static function serverCache(TaggableCacheItemPoolInterface $pool, $streamFactory, array $config = [])
    {
        return new self($pool, $streamFactory, $config);
    }
    ...
    protected function doHandleRequest(RequestInterface $request, callable $next, callable $first)
    {
        ...
        return $next($request)->then(function (ResponseInterface $response) use ($request, $cacheItem) {
            ...
            if ($this->isCacheable($response) && $this->isCacheableRequest($request)) {
                ...
                if (!empty($request->getHeader($this->config['tags_request_header']))) {
                    $cacheItem->setTags($request->getHeader($this->config['tags_request_header']));
                }
